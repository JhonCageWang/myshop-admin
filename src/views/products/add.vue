<template>
  <main-panel title="添加商品" backPath="/products">
        ...
        <el-row>
          <el-form ref="form" :rules="rules" :model="form" label-width="80px" size="small">
            ...
            <el-form-item label="物流">
              <el-radio class="radio" v-model="form.needExpress" :label="1">是</el-radio>
              <el-radio class="radio" v-model="form.needExpress" :label="0">否</el-radio>
            </el-form-item>
          ...
          <el-col :span="10" :xs="24" :sm="24" :md="10" :lg="10" style="margin-bottom: 20px;">
            ...
            <el-form ref="combinationForm" :rules="rulesAttr" :model="formSpec" label-width="80px" size="small">
              <template v-for="(sitem, index) in formSpec.specArr">
                <el-form-item label="规格名" :prop="'specArr.' + index + '.selectedSpecs'" :rules="{required: true, validator: specValidate}">
                ...
                <el-form-item label="规格值" :prop="'specArr.' + index + '.selectedSpecVal'" :rules="{required: true, message: '请填写具体规格'}">
    ...
    <el-dialog title="新增规格类型" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="addSpec" label-width="80px" size="mini" ref="addSpecForm">
...
export default {
  ...
  methods: {
    ...
    getGoodsSpecs () {
      this.tbloading = true
      this.$http.get(api.SPECIFICATION + '/goods?goodsId=' + this.form.id).then(data => {
