<template>
  <crud-panel title="规格列表">
    ...
    <el-table :data="specifications" stripe border v-loading="tbloading">
      ...
      <el-table-column prop="sortOrder" label="排序号" width="80" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    getSpecificationsList (search = false) {
      ...
      this.$http.get(api.SPECIFICATION + `/index?page=${this.page.page}&size=${this.page.size}&name=${this.keywords}`).then(data => {
        this.specifications = data.data.list
    ...
    destroy (row) {
      this.$confirm(`确定删除规格类型“${row.name}”吗?`, '提示', {type: 'warning'}).then(() => {
        this.$http.post(api.SPECIFICATION + '/delete', {id: row.id}).then(data => {
          if (data.code === 0) {
